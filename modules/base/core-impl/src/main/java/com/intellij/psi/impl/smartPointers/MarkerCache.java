  ...
  private static UpdatedRanges applyEvents(@Nonnull List<? extends DocumentEvent> events, final UpdatedRanges struct) {
    ...
    for (DocumentEvent event : events) {
      ...
      final DocumentEvent corrected = new DocumentEventImpl(frozen, event.getOffset(), event.getOldFragment(), event.getNewFragment(), event.getOldTimeStamp(), event.isWholeTextReplaced(),
                                                            ((DocumentEventImpl)event).getInitialStartOffset(), ((DocumentEventImpl)event).getInitialOldLength(), event.getMoveOffset());
